...
func ServeContent(w http.ResponseWriter, req *http.Request, name string, modtime time.Time, content io.ReadSeeker, size int64, offset int64) {
	...
	serveContent(w, req, name, modtime, sizeFunc, content, offset)
}
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker, offset int64) {
	fmt.Printf("Start serveContent\n")
	...
	case len(ranges) == 1:
		...
		ra := ranges[0]
		//ra.start += 931840
		if _, err := content.Seek(ra.start+offset, io.SeekStart); err != nil {
			http.Error(w, err.Error(), http.StatusRequestedRangeNotSatisfiable)
			return
		}
		sendSize = ra.length
		...
		fmt.Println("Set Content-Range", "size:", ra.contentRange(size))
	case len(ranges) > 1:
	...
	// The user wraps the http.ResponseWriter in one which gzips data written to it,
	...
	// of the http.ResponseWriter, and always set Content-Length if it's one
	// that we recognize.
...
func serveFile(w http.ResponseWriter, r *http.Request, fs FileSystem, name string, redirect bool) {
	...
	serveContent(w, r, d.Name(), d.ModTime(), sizeFunc, f, 0)
}
